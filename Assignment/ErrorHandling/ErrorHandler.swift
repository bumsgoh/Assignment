...
class ErrorHandler {
    ...
    final func buildErrorAlertController(error: APIError) -> UIAlertController {
        let errorAlertController = UIAlertController(title: "에러",
                                                     message: "데이터를 정상적으로 불러오지 못했습니다 Error: \(error.localizedDescription)",
                                                     preferredStyle: .alert)
        let checkingAction = UIAlertAction(title: "확인",
                                           style: .default,
                                           handler: nil)
